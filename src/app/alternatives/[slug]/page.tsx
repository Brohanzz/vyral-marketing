...
export async function generateStaticParams() {
  ...
  return alternatives.map((a) => ({ slug: a.frontmatter.slug }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!alt) return {};
  const { frontmatter: fm } = alt;
  return {
    title: `${fm.title} | VYRAL`,
    description: fm.description,
    alternates: {
      canonical: `https://www.getvyral.io/alternatives/${fm.slug}`,
    },
    openGraph: {
      title: fm.title,
      description: fm.description,
      url: `https://www.getvyral.io/alternatives/${fm.slug}`,
      type: "article",
...
const mdxComponents = {
  ...
  a: (props: React.AnchorHTMLAttributes<HTMLAnchorElement>) => (
    <a style={{ color: "#a78bfa", textDecoration: "underline" }} {...props} />
  ),
...
export default async function AlternativePage({ params }: Props) {
  ...
  const { frontmatter: fm, content } = alt;
